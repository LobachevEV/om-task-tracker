...
- **AX-tree reduction (`plan.ai.spec.ts`):** Two complementary techniques keep both `al.check()` invocations under the 4000 input-token ceiling (baseline: ~10017 tokens per call):
  1. **CSS injection** — `page.addStyleTag({ content: '[role="gridcell"] { display: none !important; }' })` runs once before the first `al.check()`. Chromium marks hidden elements `ignored=true`; Alumnium's `toXml()` serialiser skips them entirely. This removes ~75 Gantt timeline date-column gridcells (~1066 tokens) from the AX tree that Alumnium serialises for each assertion.
  2. **Attribute filtering** — `EXCLUDE_ATTRIBUTES` (exported from `e2e/helpers/llm.ts`) is passed into the `Alumni` constructor via `excludeAttributes`. Alumnium's Chromium CDP path serialises ARIA semantic properties (not HTML DOM attributes), so the filter targets CDP `AXPropertyName` values: `name`, `focusable`, `focused`, `url`, `invalid`, `readonly`, `required`, `editable`, `settable`, `multiline`, `orientation`, `autocomplete`, `pressed`, `level`, `hasPopup`, `live`, `atomic`, `relevant`. Structural ARIA properties (`role`, `value`, `checked`, `disabled`, `expanded`) and node text content are kept so the LLM retains full semantic context about interactive controls (~239 tokens saved).
  Both techniques only affect what Alumnium serialises; the real DOM and the default Playwright POM suite (`specs/`) are unchanged. Expected post-reduction token count: ~3295 per call.
