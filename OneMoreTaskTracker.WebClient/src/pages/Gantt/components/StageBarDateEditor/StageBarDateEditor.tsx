...
export interface StageBarDateEditorProps {
  ...
  onFail?: (error: InlineEditorError, range: { plannedStart: string | null; plannedEnd: string | null }) => void;
  onClearError?: () => void;
  children: ReactNode;
  dataTestId: string;
}
...
export function StageBarDateEditor({
  ...
  const handleFail = useCallback(
    (err: unknown) => {
      onFail?.(toInlineEditorError(err), lastAttemptedRangeRef.current ?? { plannedStart: null, plannedEnd: null });
    },
    [onFail],
  );
  ...
  const onPointerDown = useCallback(
    ...
    [onClearError, rawOnPointerDown],
  );

  const onKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLElement>) => {
      if (kbState.phase === 'idle') onClearError?.();
      rawOnKeyDown(e);
    },
    [kbState.phase, onClearError, rawOnKeyDown],
  );
