...
public sealed class GetAssigneeTaskSummaryHandlerTests
{
    ...
    {
        var db = CreateDb();
        var task1 = new Task { JiraId = "TASK-1", UserId = 1 }.WithFeature(1);
        task1.SetStateForTesting((int)TaskState.InDev);
        var task2 = new Task { JiraId = "TASK-2", UserId = 1 }.WithFeature(1);
    ...
    {
        var db = CreateDb();
        var t1 = new Task { JiraId = "TASK-1", UserId = 1 }.WithFeature(1);
        t1.SetStateForTesting((int)TaskState.InDev);
        var t2 = new Task { JiraId = "TASK-2", UserId = 1 }.WithFeature(1);
        t2.SetStateForTesting((int)TaskState.MrToRelease);
        var t3 = new Task { JiraId = "TASK-3", UserId = 1 }.WithFeature(1);
        t3.SetStateForTesting((int)TaskState.InTest);
        var t4 = new Task { JiraId = "TASK-4", UserId = 1 }.WithFeature(1);
        t4.SetStateForTesting((int)TaskState.MrToMaster);
        var t5 = new Task { JiraId = "TASK-5", UserId = 1 }.WithFeature(1);
    ...
    public async System.Threading.Tasks.Task BatchGetAssigneeTaskSummary_IssuedAsSingleGroupedQuery()
    {
        ...
        var t1 = new Task { JiraId = "TASK-1", UserId = 1 }.WithFeature(1);
        t1.SetStateForTesting((int)TaskState.InDev);
        var t2 = new Task { JiraId = "TASK-2", UserId = 2 }.WithFeature(1);
        t2.SetStateForTesting((int)TaskState.InTest);
        var t3 = new Task { JiraId = "TASK-3", UserId = 3 }.WithFeature(1);
