    ...
    ILogger<TasksController> logger) : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<TaskResponse>>> GetTasks(CancellationToken cancellationToken)
    {
        ...
        return Ok(response.Tasks.Select(t => new TaskResponse(t.Id, t.JiraTaskId, MapState(t.State), t.UserId)));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return Ok(new TaskDetailResponse(jiraId, MapState(response.State), projects, mergeRequests));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return Ok(new TaskResponse(lastTask.Id, lastTask.JiraTaskId, MapState(lastTask.State), User.GetUserId()));
    }
    ...
    private string MapState(TaskState state)
    {
        return state switch
        {
    ...
    private string LogAndReturnUnknown(TaskState state)
    {
